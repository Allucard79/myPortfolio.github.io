...
$(function() {
  ...
  const customErrors = {
    tooShort: (fieldName, min) =>
      "Field *" + fieldName + "* must contain at least " + min + " characters.",
    tooLong: (fieldName, max) =>
      "Field *" + fieldName + "* may contain up to " + max + " characters.",
    empty: fieldName => "Field *" + fieldName + "* cannot be empty.",
  ...
  $(".emailFormSubmit").click(event => {
    ...
    if (isValid === true) {
      ...
      sendEmail.done(response => {
        ...
        formAlert.innerHTML = "Message was sent!";
      });
    } else {
      formAlert.innerHTML = customErrors.errorsInForm;
    }
  ...
  function clearErrors() {
    ...
    this.parentElement.removeChild(
      this.parentElement.getElementsByClassName("error")[0]
    );
    formAlert.innerHTML = "";
  }
